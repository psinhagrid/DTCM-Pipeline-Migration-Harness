"""
ASSESS Agent — discovers HQL files, parses structure, extracts lineage,
scores complexity, writes to Neo4j, queries blast radius and wave.
"""
...
async def run_assess(pipeline_name: str) -> dict:
    ...
    try:
        result = await asyncio.to_thread(
            ...
            query={
                ...
                "instruction": (
                    "Run the ASSESS phase for this pipeline. "
                    "Start by calling read_skill('repo_scan') — it tells you exactly which tools "
                    "to call and in what order. Follow its guidance step by step. "
                    "Call FINAL() with the full assessment result when done."
                ),
            },
            tools=TOOLS,
            config=CONFIG,
    ...
    await _e(f"ASSESS {outcome} — {output.get('summary', '')}", "complete")
    return output
