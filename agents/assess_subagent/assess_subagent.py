...
TOOLS = [
    ...
    },
    {
        ...
        "description": "Persist the pipeline lineage graph to Neo4j. Call after lineage is extracted.",
        "input_schema": {
            "type": "object",
            "properties": {
                "nodes": {"type": "integer", "description": "Number of graph nodes"},
                "edges": {"type": "integer", "description": "Number of graph edges"},
            },
...
async def _execute_tool(name: str, args: dict, pipeline_dir: Path) -> dict:
    if name == "read_skill_tool":
        ...
        neo4j_write_graph_tool(pipeline_dir.name, args.get("nodes", 0), args.get("edges", 0))
        return {"status": "ok", "note": "stubbed — TODO: neo4j MCP"}
