...
from agents.reconcile_agent.tools_impl.utils.validators import (
...
def compile_report_tool(
    ...
    """
    # Coerce all items to strings — agent sometimes passes dicts
    all_issues = [str(x) if not isinstance(x, str) else x for x in (all_issues or [])]
