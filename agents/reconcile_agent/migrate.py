"""
RECONCILE Agent — validates the conversion: semantic comparison, runtime validation,
compiles the confidence report. Receives ASSESS + CONVERT output as context.
"""
...
from .tools import (
    read_skill,
    analyze_file, workflow_parity, runtime_validation, compile_report,
    query_graph,
)
...
async def run_reconcile(pipeline_name: str, assessment: dict, conversion: dict) -> dict:
    async def _e(msg: str, etype: str = "status") -> None:
        await push(type=etype, agent=NAME, message=msg, pipeline=pipeline_name)

    await _e(f"RECONCILE started — {pipeline_name}", "delegation")
    ...
    output_schema = {
        "type": "object",
        "required": ["pipeline", "outcome", "summary", "reconciliation"],
        "properties": {
    ...
    try:
        result = await asyncio.to_thread(
            ...
            query={
                ...
                "instruction": (
                    "Run the RECONCILE phase for this pipeline. "
                    "Start by calling read_skill('semantic_comparison') — it tells you exactly which "
                    "tools to call and in what order. Assessment is in context['assessment'], "
                    "conversion in context['conversion']. "
                    "Call FINAL() with the full reconciliation result when done."
                ),
            },
            tools=TOOLS,
            config=CONFIG,
    ...
    if not output.get("outcome"):
        output["outcome"] = "SUCCESS" if output.get("reconciliation") else "FAILED"
    outcome = output.get("outcome", "FAILED")
    await _e(f"RECONCILE {outcome} — {output.get('summary', '')}", "complete")
    return output
