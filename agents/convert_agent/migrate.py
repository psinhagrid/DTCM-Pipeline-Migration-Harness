"""
CONVERT Agent — transforms HQL files to PySpark, validates output,
generates the Airflow DAG. Receives ASSESS output as context.
"""
...
from .tools import (
    read_skill,
    transform_hql, validate_pyspark, generate_dag,
    query_graph,
)
...
async def run_convert(pipeline_name: str, assessment: dict) -> dict:
    async def _e(msg: str, etype: str = "status") -> None:
        await push(type=etype, agent=NAME, message=msg, pipeline=pipeline_name)

    await _e(f"CONVERT started — {pipeline_name}", "delegation")
    ...
    output_schema = {
        "type": "object",
        "required": ["pipeline", "outcome", "summary", "conversion"],
        "properties": {
    ...
    try:
        result = await asyncio.to_thread(
            ...
            query={
                ...
                "instruction": (
                    "Run the CONVERT phase for this pipeline. "
                    "Start by calling read_skill('hiveql_to_pyspark') — it tells you exactly which "
                    "tools to call and in what order. The assessment is in context['assessment']. "
                    "Call FINAL() with the full conversion result when done."
                ),
            },
            tools=TOOLS,
            config=CONFIG,
    ...
    if not output.get("outcome"):
        output["outcome"] = "SUCCESS" if output.get("conversion") else "FAILED"
    outcome = output.get("outcome", "FAILED")
    await _e(f"CONVERT {outcome} — {output.get('summary', '')}", "complete")
    return output
