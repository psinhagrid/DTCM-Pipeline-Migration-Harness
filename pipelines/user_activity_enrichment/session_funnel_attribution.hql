...
SELECT
    deduped.session_id,
    deduped.user_id,
    deduped.first_touch_campaign,
    deduped.last_touch_campaign,
    deduped.first_touch_channel,
    deduped.last_touch_channel,
    deduped.session_start,
    deduped.session_end,
    deduped.attributed_campaign,
    deduped.page_count,
    deduped.is_bounce,
    deduped.dt
FROM (
    SELECT
        ...
        attr.dt,
        ROW_NUMBER() OVER (
            PARTITION BY attr.session_id
            ORDER BY attr.first_touch_campaign
        )                                                AS rn
    FROM (
        SELECT
            tp.session_id,
