...
```python
# Generated by convert_subagent (Claude Sonnet)
...
result.writeTo("glue_catalog.dtcm.revenue_daily") \
...
**Example 3 — CREATE TABLE → Iceberg**
...
```python
spark.sql("""
    CREATE TABLE IF NOT EXISTS glue_catalog.dtcm.raw_events (
        event_id STRING,
        user_id STRING,
        event_type STRING
    )
